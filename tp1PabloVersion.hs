...
objetos_en_posesión_de name universo = filter (en_posesión_de name) (objetos_en universo)

{-Ejercicio 5-}

-- Asume que hay al menos un objeto
objeto_libre_mas_cercano :: Personaje -> Universo -> Objeto
objeto_libre_mas_cercano personaje universo = foldl1 (\res actual -> if distanciaAlPJ actual < distanciaAlPJ res then actual else res) (objetos_libres_en universo)
    where distanciaAlPJ = distancia (Left personaje) . Right

{-Ejercicio 6-}

tiene_thanos_todas_las_gemas :: Universo -> Bool
tiene_thanos_todas_las_gemas universo = length (filter es_una_gema (objetos_en_posesión_de "Thanos" universo)) == 6

{-Ejercicio 7-}

podemos_ganarle_a_thanos :: Universo -> Bool
podemos_ganarle_a_thanos uni = not (tiene_thanos_todas_las_gemas uni) && 
    ((está_el_personaje "Thor" uni && está_el_objeto "StormBreaker" uni) || 
    (está_el_personaje "Wanda" uni && está_el_personaje "Vision" uni && en_posesión_de "Vision" (objeto_de_nombre "Gema de la Mente" uni))) 
